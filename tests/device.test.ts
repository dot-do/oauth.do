...
describe('WorkOS Device Authorization Flow', () => {
  ...
  describe('authorizeDevice()', () => {
    it('should make correct POST request to WorkOS device auth endpoint', async () => {
      const mockFetch = vi.fn().mockResolvedValue({
        ...
        json: async () => ({
          ...
          verification_uri_complete: `https://id.org.ai/device?user_code=${mockUserCode}`,
          expires_in: 600,
          interval: 5,
        }),
      })
      ...
      expect(mockFetch).toHaveBeenCalledWith('https://id.org.ai/oauth/device', {
        method: 'POST',
        headers: {
    ...
    it('should return device_code, user_code, verification_uri, etc.', async () => {
      ...
      expect(result.verification_uri_complete).toBe(`https://id.org.ai/device?user_code=${mockUserCode}`)
    ...
    it('should handle missing client ID error', async () => {
      ...
      await expect(authorizeDevice()).rejects.toThrow('Client ID is required for device authorization')
    })
    ...
    it('should throw error on failed request', async () => {
      ...
      await expect(authorizeDevice()).rejects.toThrow('Device authorization failed: Bad Request - Invalid client_id')
    })
  })
  ...
  describe('pollForTokens()', () => {
    ...
    it('should handle access_denied response (rejects)', async () => {
      ...
      await expect(pollForTokens(mockDeviceCode, 0.05, 10)).rejects.toThrow('Access denied by user')
    })
    ...
    it('should handle expired_token response (rejects)', async () => {
      ...
      await expect(pollForTokens(mockDeviceCode, 0.05, 10)).rejects.toThrow('Device code expired')
    })
    ...
    it('should make correct POST request to authenticate endpoint', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('https://id.org.ai/oauth/token', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/x-www-form-urlencoded',
        },
    ...
    it('should throw error when client ID is missing', async () => {
      ...
      await expect(pollForTokens(mockDeviceCode, 0.05, 10)).rejects.toThrow('Client ID is required for token polling')
    })
    ...
    it('should handle unknown error responses', async () => {
      ...
      await expect(pollForTokens(mockDeviceCode, 0.05, 10)).rejects.toThrow('Token polling failed: unknown')
    })
