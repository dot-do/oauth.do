...
const mockGetSignInUrl = vi.fn().mockResolvedValue('https://id.org.ai/sign-in')
const mockGetSignUpUrl = vi.fn().mockResolvedValue('https://id.org.ai/sign-up')
...
vi.mock('@mdxui/auth', () => ({
  ...
  useAuth: () => mockUseAuth(),
  SignInButton: ({ children, className }: { children?: React.ReactNode; className?: string }) => <button className={className}>{children ?? 'Sign In'}</button>,
  ...
  TeamSwitcher: ({ className }: { className?: string }) => (
    <div data-testid='team-switcher' className={className}>
      TeamSwitcher
    </div>
  ),
...
describe('React Components', () => {
  ...
  describe('useOAuthDoConfig hook', () => {
    ...
    it('returns overridden values from provider', () => {
      render(
        <OAuthDoProvider clientId='overridden-client' apiUrl='https://overridden.apis.do' authKitDomain='overridden.oauth.do'>
          <ConfigDisplay />
        </OAuthDoProvider>,
      )
