...
describe('TokenStorage', () => {
  ...
  describe('SecureFileTokenStorage', () => {
    ...
    it('should set restrictive file permissions (0600) on the canonical token file', async () => {
      const testToken = `test-token-${Date.now()}`
      await storage.setToken(testToken)

      const tokenPath = join(process.env.HOME || homedir(), '.id.org.ai', 'token')
    ...
    it('should mirror default writes to the legacy oauth.do path', async () => {
      const testToken = `test-token-${Date.now()}`
      await storage.setToken(testToken)

      const canonicalPath = join(process.env.HOME || homedir(), '.id.org.ai', 'token')
      const legacyPath = join(process.env.HOME || homedir(), '.oauth.do', 'token')

      const [canonicalRaw, legacyRaw] = await Promise.all([readFile(canonicalPath, 'utf-8'), readFile(legacyPath, 'utf-8')])
      expect(JSON.parse(canonicalRaw).accessToken).toBe(testToken)
      expect(JSON.parse(legacyRaw).accessToken).toBe(testToken)
    })
