...
vi.mock('../src/storage.js', () => {
  const mockStorage = {
    ...
    getTokenData: vi.fn(),
    removeToken: vi.fn(),
    getStorageInfo: vi.fn().mockResolvedValue({ type: 'file', secure: true, path: '~/.id.org.ai/token' }),
  }
  return {
...
describe('CLI', () => {
  ...
  describe('loginCommand (lines 133-196)', () => {
    const mockAuthResponse = {
      ...
      user_code: 'WXYZ-5678',
      verification_uri: 'https://id.org.ai/device',
      verification_uri_complete: 'https://id.org.ai/device?user_code=WXYZ-5678',
      expires_in: 600,
      interval: 5,
    }
    ...
    it('should complete successful login flow', async () => {
      ...
      expect(pollForTokens).toHaveBeenCalledWith(mockAuthResponse.device_code, mockAuthResponse.interval, mockAuthResponse.expires_in)
  ...
  describe('autoLoginOrShowUser (lines 325-358)', () => {
    ...
    it('should trigger login flow if token is expired', async () => {
      ...
      ;(getUser as Mock).mockResolvedValueOnce({ user: null }).mockResolvedValueOnce({
      ...
      const mockAuthResponse = {
        ...
        user_code: 'AUTO-1234',
        verification_uri: 'https://id.org.ai/device',
        verification_uri_complete: 'https://id.org.ai/device?user_code=AUTO-1234',
        expires_in: 600,
        interval: 5,
      }
    ...
    it('should trigger login flow if no token exists', async () => {
      ...
      const mockAuthResponse = {
        ...
        user_code: 'NEW-5678',
        verification_uri: 'https://id.org.ai/device',
        verification_uri_complete: 'https://id.org.ai/device?user_code=NEW-5678',
        expires_in: 600,
        interval: 5,
      }
    ...
    it('should handle auth check failure and trigger login', async () => {
      ...
      ;(getUser as Mock).mockRejectedValueOnce(new Error('Auth check failed')).mockResolvedValueOnce({
      ...
      const mockAuthResponse = {
        ...
        verification_uri: 'https://id.org.ai/device',
        verification_uri_complete: 'https://id.org.ai/device?user_code=RETRY-9999',
        expires_in: 600,
        interval: 5,
      }
