...
${colors.cyan}Environment Variables:${colors.reset}
  ...
  OAUTH_AUTHKIT_DOMAIN   AuthKit domain (default: id.org.ai)
  OAUTH_API_URL          API base URL (default: https://id.org.ai)
...
async function loginCommand() {
  try {
    ...
    if (typeof fileStorage.getStorageInfo === 'function') {
      ...
      if (info.path) {
        console.log(`\n${colors.dim}Primary token store: ${colors.green}${info.path}${colors.reset}`)
      }
      console.log(`${colors.dim}Canonical callback: ${colors.green}${DEFAULT_CALLBACK_URL}${colors.reset}`)
    }
  } catch (error) {
...
async function statusCommand() {
  try {
    ...
    if (typeof fileStorage.getStorageInfo === 'function') {
      ...
      if (info.path) {
        console.log(`  ${colors.dim}Primary path: ${info.path}${colors.reset}`)
      }
      console.log(`  ${colors.dim}Mirrors legacy ~/.oauth.do/token for compatibility${colors.reset}`)
    }
